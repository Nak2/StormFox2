...
local function ApplyWeather(sName, nPercentage, nDelta)
	...
	if nPercentage >= 1 then
		for _,key in ipairs( StormFox.Weather.GetKeys() )
...
if SERVER then
	...
	net.Receive("stormfox.weather", function(len)
		...
		local nDelta = net.WriteInt(nDelta, 8)
